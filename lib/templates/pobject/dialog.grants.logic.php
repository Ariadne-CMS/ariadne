<?php
	...
	if( !function_exists("getPathByType") ) {
		function getPathByType($type, $id) {
			...
			switch ($type) {
				case 'pgroup':
					$path  = current($AR->user->find('/system/groups/', 'login.value=\''.$id.'\'', 'system.get.path.phtml'));
					break;
				case 'puser':
					$path  = current($AR->user->find('/system/users/', 'login.value=\''.$id.'\'', 'system.get.path.phtml'));
					break;
			}
			return $path;
		}
