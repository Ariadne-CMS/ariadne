<?php
	...
	if (!isset($ARCurrent->arDontCache)) {
		$ARCurrent->arDontCache = false;
	}
	if (!$ARCurrent->arDontCache && !isset($ARCurrent->cacheCallChainSettings[$this->id])) {
		$ARCurrent->cacheCallChainSettings[$this->id] = array (
			"serverCache"        => -2,
		);
	}
	...
	if (is_array($files)) {
		...
		if (isset($notfound) && is_array($notfound)) {
