			...
			foreach ($selectedob->data->config->grants as $type => $grant) {
				foreach ($grant as $id => $grants) {
					if ($type == 'pgroup') {
						...
						$name  = current($this->find('/system/groups/', 'login.value=\''.$id.'\'', 'system.get.name.phtml'));
					} else {
						// type is puser;
						$path  = current($this->find('/system/users/', 'login.value=\''.$id.'\'', 'system.get.path.phtml'));
						$name  = current($this->find('/system/users/', 'login.value=\''.$id.'\'', 'system.get.name.phtml'));
					}
					...
					foreach ($grants as $grantname => $grantvalue) {
						if (!isset($available_grants[$grantname])) {
							$available_grants[$grantname] = labelspan($grantname, 8);
						}
...
?>
<div class="items">
	<h2><?php echo $ARnls['ariadne:grants:users_with_grants']; echo labelspan($selectedpath, 20); ?></h2>
	<?php if ($error) { ?>
...
<div class="browse">
	...
	<input class="button" type="button" value="..." title="<?php echo $ARnls['browse']; ?>" onclick='callbacktarget="extrauser"; window.open("<?php echo $this->make_local_url('/'); ?>" + document.getElementById("extrauser").value + "dialog.browse.php", "browse", "height=480,width=750"); return false;'>
