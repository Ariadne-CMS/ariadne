...
function tableEditor(tWindow, tElement) {

   this.document = tWindow.document; // reference to editable this.document
   this.window = tWindow;
   this.element = tElement;   // reference to the editable region if any
   ...
   this.arrowStyle = "width: 7px; height: 7px; overflow: hidden; background-color: white; margin: 0px; padding: 0px; border: 1px solid black;";
   this.callBack = 'parent.tEdit';
   ...
   var rArrowHTML='<div id="rArrow" title="Drag to modify cell width." style="'+this.arrowStyle+'position:absolute; visibility:hidden; cursor: E-resize; z-index: 1" onmousedown="'+this.callBack+'.markCellResize(this)" onmouseup="'+this.callBack+'.stopCellResize(false)" ondragstart="'+this.callBack+'handleDrag(0)"></div>';
   var dArrowHTML='<div id="dArrow" title="Drag to modify cell height." style="'+this.arrowStyle+'position:absolute; visibility:hidden; cursor: S-resize; z-index: 1" onmousedown="'+this.callBack+'.markCellResize(this)" onmouseup="'+this.callBack+'.stopCellResize(false)" ondragstart="'+this.callBack+'handleDrag(0)"></div>';
   this.document.body.insertAdjacentHTML('beforeEnd',rArrowHTML);
   this.document.body.insertAdjacentHTML('beforeEnd',dArrowHTML);
   this.rArrow=this.document.getElementById('rArrow');
   this.dArrow=this.document.getElementById('dArrow');
   ...
   function setTableElements(){
      ...
      cursorPos=this.document.selection.createRange();

      if ((this.document.selection.type == 'Text') || (this.document.selection.type == 'None')) {
         var elt = cursorPos.parentElement(); 
         while (elt) {
            if (elt.className=='editable') {
               return;
            }
            if (elt.tagName == "TD") {
               break;
         ...
         if (elt) {
            // don't select this.document area
            if (elt == this.document)
               return;
            ...
            if (this.element) {
               ...
               // don't select editable region
               if (this.element.className=='editable')
                  return;
            }

            // check that we're in an editable region
            var parent=elt;
            while (parent) {
   ...
   function unMergeDown() {
      ...
      this.tableCell.parentNode.parentNode.childNodes[ topRowIndex + this.tableCell.rowSpan - 1 ].appendChild( this.document.createElement("TD") );
   ...
   function splitCell() {
      ...
      var newCell = this.tableCell.parentNode.insertBefore( this.document.createElement("TD"), this.tableCell);
   ...
   function addCell() {
      ...
      this.tableCell.parentElement.insertBefore(this.document.createElement("TD"), this.tableCell.nextSibling);
   }
   ...
   function processRow(action) {
      ...
      if (!tbl) {
//         top.status="Could not " + action + " row.";
         return;
      }
      ...
      if (action == "add") {
         ...
         for (var i = 0; i < numcells; i++) {
            var c = r.appendChild( this.document.createElement("TD") );
   ...
   function processColumn(action) {
      ...
      if (!tbl) {
//         top.status="Could not " + action + " column.";
         return;
      }
   ...
   function exploreTree(obj, pnt) {
      if (!obj.childNodes.length)
         return;
      var i;
      var ul = pnt.appendChild( this.document.createElement("UL") );
      for (i = 0; i < obj.childNodes.length; i++) {
         ...
         /*
         var n = this.document.createTextNode(obj.childNodes[i].tagName);
         li.appendChild(n);
         */
      }
   }
   ...
   function explore(obj, pnt) {
      var i;
      for (i in obj) {
         var n = this.document.createTextNode(i +"="+obj[i]);
         pnt.appendChild(n);
         pnt.appendChild( this.document.createElement("BR") );
      }
   }
   ...
   function buildTable(pnt) {
      var t = pnt.appendChild( this.document.createElement("TABLE") );
      t.border=1;
      t.cellPadding=2;
      t.cellSpacing=0;
      var tb = t.appendChild( this.document.createElement("TBODY") );
      for(var r = 0; r < 10; r++) {
         var tr = tb.appendChild( this.document.createElement("TR") );
         for(var c = 0; c < 10; c++) {
            var cell = tr.appendChild( this.document.createElement("TD") );
            cell.appendChild( this.document.createTextNode(r+"-"+c) );
         }
      }
   }
   ...
   function changePos() {
      ...
      this.moveable.style.posTop = this.window.event.clientY - 10;
      this.moveable.style.posLeft = this.window.event.clientX - 25;
   }
   ...
   function markCellResize(obj) {
      if (this.cellResizeObj) {
         ...
         this.document.onselectstart = rfalse;
      }
   }
...
//      var rArrow=this.document.getElementById("rArrow");
//      var dArrow=this.document.getElementById("dArrow");
      if (this.rArrow) {
         this.rArrow.style.visibility = 'hidden';
      }
      if (this.dArrow) {
         this.dArrow.style.visibility = 'hidden';
      }
   }
...
//      var scrollLeft = this.document.body.scrollLeft;
/*
      this.document.getElementById("rArrow").style.posLeft = cell_wdt + this.tableCell.offsetWidth - 6 - scrollLeft;
      this.document.getElementById("rArrow").style.posTop = cell_hgt + (this.tableCell.offsetHeight / 2) - 2 - scrollTop;

      this.document.getElementById("dArrow").style.posLeft = cell_wdt + (this.tableCell.offsetWidth / 2) - 2 - scrollLeft;
      this.document.getElementById("dArrow").style.posTop = cell_hgt + this.tableCell.offsetHeight - 6 - scrollTop;
*/

//      var rArrow=this.document.getElementById("rArrow");
//      var dArrow=this.document.getElementById("dArrow");
      if (this.rArrow) {
         this.rArrow.style.posLeft = cell_wdt + this.tableCell.offsetWidth - 3 - scrollLeft;
         this.rArrow.style.posTop = cell_hgt + (this.tableCell.offsetHeight / 2) - 2 - scrollTop;
         this.rArrow.style.visibility = 'visible';
      }
      if (this.dArrow) {
         this.dArrow.style.posLeft = cell_wdt + (this.tableCell.offsetWidth / 2) - 2 - scrollLeft;
         this.dArrow.style.posTop = cell_hgt + this.tableCell.offsetHeight - 3 - scrollTop;
         this.dArrow.style.visibility = 'visible';
      }
   }
   ...
   function repositionArrows() {
      ...
      var cell_hgt = this.tableCell.offsetTop;
      var cell_wdt = this.tableCell.offsetLeft;
      /* IE gives an 'unspecified error' sometimes, but par is still set to something... weird */
      try {
        var par = this.tableCell.offsetParent;
      } catch(e) {
        par=false;
...
/*
      this.document.getElementById("rArrow").style.posLeft = cell_wdt + this.tableCell.offsetWidth - 3 - scrollLeft;
      this.document.getElementById("rArrow").style.posTop = cell_hgt + (this.tableCell.offsetHeight / 2) - 2 - scrollTop;

      this.document.getElementById("dArrow").style.posLeft = cell_wdt + (this.tableCell.offsetWidth / 2) - 2 - scrollLeft; 
      this.document.getElementById("dArrow").style.posTop = cell_hgt + this.tableCell.offsetHeight - 3 - scrollTop;
*/
   }
   ...
   function resizeCell() {
      ...
      if (this.cellResizeObj.id == 'dArrow') {
         var scrollTop = this.document.body.scrollTop;
         var newHeight = (this.window.event.clientY - (this.cellY - scrollTop) ) + this.cellHeight;
         ...
         var scrollLeft = this.document.body.scrollLeft;
         var newWidth = (this.window.event.clientX - (this.cellX - scrollLeft) ) + this.cellWidth;
   ...
   function resizeColumn(size) {
      ...
      if (!tbl) {
//         top.status="Could not resize  column.";
         return;
      }
