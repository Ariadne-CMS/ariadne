...
<?php
	function labelspan($label, $maxlabellength=24) {
...
<h2><?php echo $ARnls['ariadne:grants:objects_with_grants']; ?></h2>
	...
	?>
			<label for="select_<?php echo $ob_id; ?>" class="block item <?php if($path == $selectedpath) { echo " selected";} ?>" title="<?php echo $info['name'];?>">
				...
				<span class="path"><?php echo labelspan($path); ?></span>
			</label>
...
<div class="browse">
	...
	<input class="button" type="button" value="..." title="<?php echo $ARnls['browse']; ?>" onclick='callbacktarget="extrapath"; window.open("<?php echo $this->make_local_url('/'); ?>" + document.getElementById("extrapath").value + "dialog.browse.php", "browse", "height=480,width=750"); return false;'>
