...
class sqlite_compiler extends sql_compiler {
			...
			case 'cmp':
				...
				if ($node["left"]["id"]!=="implements") {
					...
					switch ($operator) {
						...
						break;
						default:
							$result = $this->tbl_prefix."objects.vtype";
							$result = "SUBSTR( $result, 0, ( INSTR( $result, '.' ) ) )";
							$result = " ( $result in (select type from ".$this->tbl_prefix."types where implements $operator $type )) ";
							$result = " ( $result OR ( " . $this->tbl_prefix."objects.vtype in (select type from ".$this->tbl_prefix."types where implements $operator $type ) ) ) ";
//							echo $result . "\n\n";
						break;
					}
				}
	...
	protected function priv_sql_compile($tree) {
		...
		$select_query = "select distinct($nodes.path), $objects.id, $nodes.parent, $nodes.priority, $objects.type, ".
		                " $objects.lastchanged as lastchanged, $objects.vtype ";
		...
		$result = array("select_query" => $select_query, "count_query" => $count_query);
		return $result;
	}
