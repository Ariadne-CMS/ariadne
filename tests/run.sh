...
if [ "${ENCRYPT:-no}" == "yes" ] ; then
	...
	echo '$AR->DB->crypto = array("test1" => array( "method" => "ar_security_crypt", "token" => "test1", "key" => $AR->sessionCryptoKey, "paths" => array("/projects/")));' | tee -a  ${BUILDROOT}/lib/configs/ariadne.phtml
	cat "${BUILDROOT}/lib/configs/ariadne.phtml"
	php -l  "${BUILDROOT}/lib/configs/ariadne.phtml"
fi
